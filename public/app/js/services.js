...
angular.module('bootstrapVariablesEditor.services', []).
  ...
  factory('ap_less', function() {
    ...
    lessEngine.saveLessVar = function(data){
        /*
        var $formAlby = $('<form>').attr('method', 'POST').attr('action', 'http://localhost:3000/app/save/')
            .append('<input>')
                .attr('type','hidden')
                .attr('name','lessObj')
                .attr('value',data);
            console.log(data);
        */
        $.post('http://localhost:3000/app/save/',{lessObj:data},function(otherData){
            alert(otherData.success!=null ? otherData.success : otherData.error);
        },'json');
        /*
